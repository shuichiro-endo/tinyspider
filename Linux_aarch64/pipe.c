...
void push_routing_message_pipe(tid_t thread_id, struct stack_head *stack)
{
...
void push_socks5_message_pipe(tid_t thread_id, struct stack_head *stack)
{
    struct push_message_queue_function_args_pipe *args = stack->args;
    struct pipe_data *pipe = args->pipe;
    struct spider_message *message = args->message;
...
void pipe_recv_message(tid_t thread_id, struct stack_head *stack)
{
    ...
    long val = 0;
    int tmp = 0;
    int *join_futex = (int *)&stack->join_futex;
    ...
    (
        ...
        "ldaxr %x0, [%[join_futex]]\n"
        "add %x0, %x0, #0x1\n"
        "stlxr %w1, %x0, [%[join_futex]]\n"
        "cbnz %w1, pipe_recv_message_1"
        : "=&r"(val),
          "=&r"(tmp),
          [join_futex] "+r"(join_futex)
        :
        : "memory", "cc"
    );
...
void pipe_send_routing_message(tid_t thread_id, struct stack_head *stack)
{
    struct pipe_data *pipe = stack->args;
    struct map_node_thread *thread = NULL;
    int ret = 0;
    ...
    __asm__ __volatile__
    (
        "pipe_send_routing_message_1:\n"
        "ldaxr %x0, [%[join_futex]]\n"
        "add %x0, %x0, #0x1\n"
        "stlxr %w1, %x0, [%[join_futex]]\n"
        "cbnz %w1, pipe_send_routing_message_1"
        : "=&r"(val),
          "=&r"(tmp),
          [join_futex] "+r"(join_futex)
        :
...
void pipe_send_socks5_message(tid_t thread_id, struct stack_head *stack)
{
    struct pipe_data *pipe = stack->args;
    struct map_node_thread *thread = NULL;
    int ret = 0;
    ...
    __asm__ __volatile__
    (
        "pipe_send_socks5_message_1:\n"
        "ldaxr %x0, [%[join_futex]]\n"
        "add %x0, %x0, #0x1\n"
        "stlxr %w1, %x0, [%[join_futex]]\n"
        "cbnz %w1, pipe_send_socks5_message_1"
        : "=&r"(val),
          "=&r"(tmp),
          [join_futex] "+r"(join_futex)
        :
...
void pipe_worker(tid_t thread_id, struct stack_head *stack_pipe_worker)
{
...
void connect_pipe(tid_t thread_id, struct stack_head *stack)
{
    struct pipe_data *pipe = stack->args;
    struct map_node_thread *thread = NULL;
    int ret = 0;
...
void listen_pipe(tid_t thread_id, struct stack_head *stack)
{
