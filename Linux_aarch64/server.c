...
void insert_map_node_server_receive_message_thread(tid_t thread_id, struct stack_head *stack)
{
...
void push_socks5_message_server(tid_t thread_id, struct stack_head *stack)
{
...
void forwarder_recv_data_server(tid_t thread_id, struct stack_head *stack)
{
    ...
    __asm__ __volatile__
    (
        "forwarder_recv_data_server_1:\n"
        "ldaxr %x0, [%[join_futex]]\n"
        "add %x0, %x0, #0x1\n"
        "stlxr %w1, %x0, [%[join_futex]]\n"
        "cbnz %w1, forwarder_recv_data_server_1"
        : "=&r"(val),
          "=&r"(tmp),
...
void forwarder_send_data_server(tid_t thread_id, struct stack_head *stack)
{
    struct server_data *server = stack->args;
    struct map_node_thread *thread = NULL;
    int ret = 0;
    ...
    __asm__ __volatile__
    (
        "forwarder_send_data_server_1:\n"
        "ldaxr %x0, [%[join_futex]]\n"
        "add %x0, %x0, #0x1\n"
        "stlxr %w1, %x0, [%[join_futex]]\n"
        "cbnz %w1, forwarder_send_data_server_1"
        : "=&r"(val),
          "=&r"(tmp),
          [join_futex] "+r"(join_futex)
        :
...
void server_worker(tid_t thread_id, struct stack_head *stack)
{
