...
void insert_map_node_client_receive_message_thread(tid_t thread_id, struct stack_head *stack)
{
...
void push_socks5_message_client(tid_t thread_id, struct stack_head *stack)
{
...
void forwarder_recv_data_client(tid_t thread_id, struct stack_head *stack)
{
    ...
    __asm__ __volatile__
    (
        "forwarder_recv_data_client_1:\n"
        "ldaxr %x0, [%[join_futex]]\n"
        "add %x0, %x0, #0x1\n"
        "stlxr %w1, %x0, [%[join_futex]]\n"
        "cbnz %w1, forwarder_recv_data_client_1"
        : "=&r"(val),
          "=&r"(tmp),
...
void forwarder_send_data_client(tid_t thread_id, struct stack_head *stack)
{
    ...
    __asm__ __volatile__
    (
        "forwarder_send_data_client_1:\n"
        "ldaxr %x0, [%[join_futex]]\n"
        "add %x0, %x0, #0x1\n"
        "stlxr %w1, %x0, [%[join_futex]]\n"
        "cbnz %w1, forwarder_send_data_client_1"
        : "=&r"(val),
          "=&r"(tmp),
          [join_futex] "+r"(join_futex)
        :
...
void client_worker(tid_t thread_id, struct stack_head *stack_client_worker)
{
...
void listen_client(tid_t thread_id, struct stack_head *stack)
{
