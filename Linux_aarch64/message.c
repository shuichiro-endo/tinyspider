...
void push_routing_message(tid_t thread_id, struct stack_head *stack)
{
    struct spider_message *message = stack->args;
    ...
    {
#ifdef _DEBUG
        printf("[-] push_routing_message cannot free stack\n");
#endif
    }
...
void push_socks5_message(tid_t thread_id, struct stack_head *stack)
{
    struct spider_message *message = stack->args;
    struct map_node_thread *thread = NULL;
    ...
    {
#ifdef _DEBUG
        printf("[-] push_socks5_message cannot free stack\n");
#endif
    }
    ...
    if(socks5_message != NULL)
    {
        if(socks5_message->header.message_type == 's')  // socks5 message
        {
            if(is_spider_ip(ip, (char *)&socks5_message->header.destination_ip))
            {
                ...
                {
                    spider_node = search_spider_node(tree_client, &sem_tree_client, socks5_message->header.client_id);
                    if(spider_node != NULL)
                    {
                        ...
                        {
                            struct insert_map_node_client_receive_message_thread_function_args *args = (insert_map_node_client_receive_message_thread_function_args *)calloc(1, sizeof(struct insert_map_node_client_receive_message_thread_function_args));
                            args->client = client;
                            args->message_id = socks5_message->header.message_id;
                            args->message = socks5_message;

                            start_thread(m_thread, (void *)insert_map_node_client_receive_message_thread, (void *)args);
                        }else
                        {
                            struct push_message_queue_function_args_client *args = (push_message_queue_function_args_client *)calloc(1, sizeof(struct push_message_queue_function_args_client));
                            args->client = client;
                            args->message = socks5_message;

                            start_thread(m_thread, (void *)push_socks5_message_client, (void *)args);
                        }
                    }else
                    {
#ifdef _DEBUG
                ...
                {
                    spider_node = search_spider_node(tree_server, &sem_tree_server, socks5_message->header.server_id);
                    if(spider_node != NULL)
                    {
                        ...
                        if(socks5_message->header.receive_flag == 1)
                        {
                            struct insert_map_node_server_receive_message_thread_function_args *args = (struct insert_map_node_server_receive_message_thread_function_args *)calloc(1, sizeof(struct insert_map_node_server_receive_message_thread_function_args));
                            args->server = server;
                            args->message_id = socks5_message->header.message_id;
                            args->message = socks5_message;

                            start_thread(m_thread, (void *)insert_map_node_server_receive_message_thread, (void *)args);
                        }else
                        {
                            struct push_message_queue_function_args_server *args = (struct push_message_queue_function_args_server *)calloc(1, sizeof(struct push_message_queue_function_args_server));
                            args->server = server;
                            args->message = socks5_message;

                            start_thread(m_thread, (void *)push_socks5_message_server, (void *)args);
                        }
                    }else
                    {
                        // generate server
            ...
            }else
            {
                ...
                if(pipe != NULL)
                {
                    struct push_message_queue_function_args_pipe *args = (struct push_message_queue_function_args_pipe *)calloc(1, sizeof(struct push_message_queue_function_args_pipe));
                    args->pipe = pipe;
                    args->message = socks5_message;
...
void message_worker(tid_t thread_id, struct stack_head *stack)
{
    bool *prevent_spider_server_startup_flag = stack->args;
    struct map_node_thread *thread = NULL;
    ...
    while(1)
    {
        ...
        if(socks5_message != NULL && *prevent_spider_server_startup_flag == false)  // generate server
        {
            ...
            create_map_server_receive_message(&server->m_server_receive_message, &server->sem_m_server_receive_message);

            do
            {
                server_id = generate_random_id();
                server->server_id = server_id;
                ret = insert_spider_node(tree_server, &sem_tree_server, server_id, server);
            }while(ret != 0);
            ...
            server_worker_function_args *args = (struct server_worker_function_args *)calloc(1, sizeof(struct server_worker_function_args));
            args->server = server;
            args->message = socks5_message;

            start_thread(m_thread, (void *)server_worker, (void *)args);
    ...
    {
#ifdef _DEBUG
        printf("[-] message_worker cannot free stack\n");
#endif
    }
