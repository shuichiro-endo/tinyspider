...
{
    struct function_args *args = (struct function_args *)calloc(1, sizeof(struct function_args));
    ...
    while(1)
    {
        ...
        ptr = fgets(stdin, buffer, INET6_ADDR_STRING_LENGTH + 1);
        if(ptr == NULL)
        {
            printf("[-] input error\n");
        ...
        }else if(strcmp(buffer, "c") == 0)  // client
        {
            ...
            {
                args = (struct function_args *)calloc(1, sizeof(struct function_args));
                args->args = (void *)pipe;
                start_thread(m_thread, (void *)connect_pipe, (void *)args);
                sleep(5);
        ...
        }else if(strcmp(buffer, "s") == 0)  // server
        {
            ...
            if(strcmp(buffer, "y") == 0)
            {
                args = (struct function_args *)calloc(1, sizeof(struct function_args));
                args->args = (void *)pipe;
                start_thread(m_thread, (void *)listen_pipe, (void *)args);
                sleep(5);
...
void add_node_spider_client()
{
    ...
    while(1)
    {
        ...
        {
            args = (struct function_args *)calloc(1, sizeof(struct function_args));
            args->args = (void *)client;
            start_thread(m_thread, (void *)listen_client, (void *)args);
            sleep(5);
...
{
    // pipe
    print_spider_node_pipe(tree_pipe, mutex_tree_pipe);

    // client
    print_spider_node_client(tree_client, mutex_tree_client);

    // server
    print_spider_node_server(tree_server, mutex_tree_server);
