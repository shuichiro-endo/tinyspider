...
int create_map_server_receive_message(struct map_server_receive_message **map, HANDLE *handle)
{
    NTSTATUS status;
    ...
    {
#ifdef _DEBUG
        printf("[-] create_map_server_receive_message NtCreateMutant error: %x\n", status);
#endif
        return -1;
    }
    ...
    {
#ifdef _DEBUG
        printf("[-] create_map_server_receive_message NtWaitForSingleObject error: %x\n", status);
#endif
        return -1;
    }
    ...
    *map = (struct map_server_receive_message *)calloc(1, sizeof(struct map_server_receive_message));
    ...
    {
#ifdef _DEBUG
        printf("[-] create_map_server_receive_message NtReleaseMutant error: %x\n", status);
#endif
        return -1;
    }
...
void insert_map_node_server_receive_message(struct map_server_receive_message *map, HANDLE handle, uint32_t message_id, struct spider_message *message)
{
    NTSTATUS status;
    LONG previousCount_mutex = 0;
    ...
    {
#ifdef _DEBUG
        printf("[-] insert_map_node_server_receive_message NtWaitForSingleObject error: %x\n", status);
#endif
        return;
    }
    ...
    {
#ifdef _DEBUG
        printf("[-] insert_map_node_server_receive_message NtReleaseMutant error: %x\n", status);
#endif
        return;
    }
}

struct map_node_server_receive_message *search_map_node_server_receive_message(struct map_server_receive_message *map, HANDLE handle, uint32_t message_id)
{
    NTSTATUS status;
    LONG previousCount_mutex = 0;
    ...
    {
#ifdef _DEBUG
        printf("[-] search_map_node_server_receive_message NtWaitForSingleObject error: %x\n", status);
#endif
        return NULL;
    }
    ...
    while(current != NULL)
    {
        if(current->message_id == message_id)
        {
            ...
            {
#ifdef _DEBUG
                printf("[-] search_map_node_server_receive_message NtReleaseMutant error: %x\n", status);
#endif
                return NULL;
            }
...
void delete_map_node_server_receive_message(struct map_server_receive_message *map, HANDLE handle, uint32_t message_id)
{
    NTSTATUS status;
    LONG previousCount_mutex = 0;
    struct map_node_server_receive_message *current = NULL;
    ...
    {
#ifdef _DEBUG
        printf("[-] delete_map_node_server_receive_message NtWaitForSingleObject error: %x\n", status);
#endif
        return;
    }
    ...
    {
#ifdef _DEBUG
        printf("[-] delete_map_node_server_receive_message NtReleaseMutant error: %x\n", status);
#endif
        return;
    }
}

void free_map_server_receive_message(struct map_server_receive_message *map, HANDLE handle)
{
    NTSTATUS status;
    LONG previousCount_mutex = 0;
    struct map_node_server_receive_message *current = NULL;
    ...
    {
#ifdef _DEBUG
        printf("[-] free_map_server_receive_message NtWaitForSingleObject error: %x\n", status);
#endif
        return;
    }
    ...
    {
#ifdef _DEBUG
        printf("[-] free_map_server_receive_message NtReleaseMutant error: %x\n", status);
#endif
        return;
    }
}

void insert_map_node_server_receive_message_thread(struct function_args *args)
{
    struct insert_map_node_server_receive_message_thread_args *insert_map_node_server_receive_message_thread_args = (struct insert_map_node_server_receive_message_thread_args *)args->args;
    struct server_data *server = insert_map_node_server_receive_message_thread_args->server;
    uint32_t message_id = insert_map_node_server_receive_message_thread_args->message_id;
    struct spider_message *message = insert_map_node_server_receive_message_thread_args->message;
    struct map_node_thread *thread = NULL;

    free(insert_map_node_server_receive_message_thread_args);

    insert_map_node_server_receive_message(server->m_server_receive_message, server->mutex_m_server_receive_message, message_id, message);
    ...
    {
#ifdef _DEBUG
        printf("[-] insert_map_node_server_receive_message_thread cannot close handle\n");
#endif
    }
...
void push_socks5_message_server(struct function_args *args)
{
    struct push_message_queue_args_server *push_message_queue_args_server = (struct push_message_queue_args_server *)args->args;
    struct server_data *server = push_message_queue_args_server->server;
    struct spider_message *message = push_message_queue_args_server->message;
    struct map_node_thread *thread = NULL;

    free(push_message_queue_args_server);
    ...
    {
#ifdef _DEBUG
        printf("[-] push_socks5_message_server cannot close handle\n");
#endif
    }
...
struct spider_message *pop_socks5_message_server_timeout(struct server_data *server, long long tv_sec, long long tv_usec)
{
...
int32_t recv_message_server(struct server_data *server, char *buffer, int32_t buffer_size, long long tv_sec, long long tv_usec)
{
    int32_t ret = 0;
    int32_t rec = 0;
    struct spider_message *socks5_message = NULL;
...
int32_t send_message_server(struct server_data *server, char *buffer, int32_t buffer_length, long long tv_sec, long long tv_usec)
{
    int32_t ret = 0;
    int32_t sen = 0;
    struct spider_message *socks5_message = NULL;
...
int recv_receive_message_server(struct server_data *server, uint32_t message_id, long long tv_sec, long long tv_usec)
{
    ...
    while(1)
    {
        node = search_map_node_server_receive_message(server->m_server_receive_message, server->mutex_m_server_receive_message, message_id);
        if(node != NULL)
        {
            ...
            if(socks5_message != NULL)
            {
                ...
                {
                    delete_map_node_server_receive_message(server->m_server_receive_message, server->mutex_m_server_receive_message, message_id);
...
int send_receive_message_server(struct server_data *server, uint32_t message_id, uint8_t receive_flag, uint8_t receive_result, long long tv_sec, long long tv_usec)
{
...
void forwarder_recv_data_server(void *args)
{
    struct server_data *server = (struct server_data *)args;
    struct map_node_thread *thread = NULL;
    int ret = 0;
    int32_t rec = 0;
    int32_t sen = 0;
...
#ifdef _DEBUG
                    printf("[+] [server -> target] forwarder_recv_data_server send(%u)\n", server->recv_message_id);
#endif

                    while(len > 0)
                    {
                        sen = send(server->target_sock, buffer + send_length, len, 0);
                        if(sen == SOCKET_ERROR)
                        {
                            err = WSAGetLastError();
...
#ifdef _DEBUG
                            printf("[-] [server -> target] forwarder_recv_data_server send error: %d\n", err);
#endif
...
void forwarder_send_data_server(void *args)
{
    struct server_data *server = (struct server_data *)args;
    struct map_node_thread *thread = NULL;
    int ret = 0;
    ...
    while(1)
    {
        ...
        {
#ifdef _DEBUG
            printf("[+] [server <- target] forwarder_send_data_server recv\n");
#endif

            memset(buffer, 0, NODE_BUFFER_SIZE);

            rec = recv(server->target_sock, buffer, SPIDER_MESSAGE_DATA_SIZE, 0);
            if(rec == SOCKET_ERROR)
            {
                err = WSAGetLastError();
                if(err == WSAEWOULDBLOCK)
...
#ifdef _DEBUG
                printf("[-] forwarder_send_data_server recv error: %d\n", err);
#endif
...
{
    NTSTATUS status;
    HANDLE handle_forwarder_recv_data_server = NULL;
    HANDLE handle_forwarder_send_data_server = NULL;

    status = NtCreateThreadEx(&handle_forwarder_recv_data_server, THREAD_ALL_ACCESS, NULL, NtCurrentProcess(), (void *)forwarder_recv_data_server, (void *)server, 0, 0, 0, 0, NULL);
    if(!NT_SUCCESS(status))
    {
#ifdef _DEBUG
        printf("[-] forwarder_server NtCreateThreadEx forwarder_recv_data_server error: %x\n", status);
#endif
    }

    status = NtCreateThreadEx(&handle_forwarder_send_data_server, THREAD_ALL_ACCESS, NULL, NtCurrentProcess(), (void *)forwarder_send_data_server, (void *)server, 0, 0, 0, 0, NULL);
    if(!NT_SUCCESS(status))
    {
#ifdef _DEBUG
        printf("[-] forwarder_server NtCreateThreadEx forwarder_send_data_server error: %x\n", status);
#endif
    }

    status = NtWaitForSingleObject(handle_forwarder_recv_data_server, false, NULL);
    if(!NT_SUCCESS(status))
    {
#ifdef _DEBUG
        printf("[-] forwarder_server NtWaitForSingleObject forwarder_recv_data_server error: %x\n", status);
#endif
    }

    status = NtWaitForSingleObject(handle_forwarder_send_data_server, false, NULL);
    if(!NT_SUCCESS(status))
    {
#ifdef _DEBUG
        printf("[-] forwarder_server NtWaitForSingleObject forwarder_send_data_server error: %x\n", status);
#endif
    }

    NtClose(handle_forwarder_recv_data_server);
    NtClose(handle_forwarder_send_data_server);
}
...
void do_socks5_connection_server(struct server_data *server, struct spider_message *socks5_message)
{
    ...
    int target_addr_length = sizeof(struct sockaddr_in);
    int target_addr6_length = sizeof(struct sockaddr_in6);
    ...
    if(atyp == 0x1) // ipv4
    {
        ...
        if(cmd == 0x1)  // CONNECT
        {
            ...
            if(server->target_sock == INVALID_SOCKET)
            {
#ifdef _DEBUG
                printf("[-] [server -> target] do_socks5_connection_server socket error: %d\n", WSAGetLastError());
#endif
            ...
            {
#ifdef _DEBUG
                printf("[-] [server <- target] do_socks5_connection_server cannot connect error: %d\n", WSAGetLastError());
#endif
                ...
                closesocket(server->target_sock);
...
void server_worker(struct function_args *args)
{
    struct server_worker_args *server_worker_args = (struct server_worker_args *)args->args;
    struct server_data *server = server_worker_args->server;
    struct spider_message *message = server_worker_args->message;
    struct map_node_thread *thread = NULL;

    free(server_worker_args);

    do_socks5_connection_server(server, message);

    free_map_server_receive_message(server->m_server_receive_message, server->mutex_m_server_receive_message);
    free_queue(server->socks5_message_queue);
    delete_spider_node(tree_server, mutex_tree_server, server->server_id);
    ...
    {
#ifdef _DEBUG
        printf("[-] server_worker cannot close handle\n");
#endif
    }
